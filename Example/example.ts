		...
		async(events) => {
			...
			if(events['messages.upsert']) {
				...
				if(upsert.type === 'notify') {
					...
					console.log('receiving msg from ' , JSON.stringify( upsert.messages ) )

					for(const msg of upsert.messages) {
						if(!msg.key.fromMe && doReplies) {
							console.log('waiting auto replying to', msg.key.remoteJid)
							//await sock!.readMessages([msg.key])
							//await sendMessageWTyping({ text: "hahaha" }, msg.key.remoteJid!)
						}
					}
				}
